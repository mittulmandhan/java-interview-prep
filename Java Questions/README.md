...
***20. Why Java is Secured Language?***
* Byte Verifier is one of the feature which makes jave a secured language.If attacker make changes in the class file manually to create some kind of virus, Bytecode verifier detects that class file as 'not generated by valid compiler'. As a result, Verification fails and we get runtime exception saying `java.lang.VerifyError`.

***21. How println method is abstracted?***
we use `System.out.println();` statement to use println method where System is the class, out is an object reference and println is the method.println is overloaded.
